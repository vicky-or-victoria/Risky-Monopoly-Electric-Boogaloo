...
class RiskyMonopolyBot(commands.Bot):
    ...
    async def on_tree_error(self, interaction: discord.Interaction, error: Exception):
        ...
        Catches discord.Forbidden anywhere in the slash-command tree and
        surfaces a clear, actionable permission report to the user instead
        of letting the error disappear silently.
        """
        if isinstance(error, discord.Forbidden):
            reason = str(error)
            if hasattr(error, 'response') and error.response is not None:
                reason = f"HTTP {error.response.status} — {reason}"

            # Build a report listing what Discord said
            report = (
                "❌ **Permission Error** — Discord rejected an action the bot tried to perform.\n\n"
                f"**What Discord said:** `{reason}`\n\n"
                "💡 **What to do:**\n"
                "  • Check that the bot's role has all necessary permissions in the relevant channel.\n"
                "  • Common culprits: *Create Forum Threads*, *Send Messages*, *Manage Threads*, *Read Messages*.\n"
                "  • If this keeps happening, an admin should open **Server Settings → Roles** and verify the bot's role.\n\n"
                f"*(Command that failed: `{interaction.command.qualified_name if interaction.command else 'unknown'}`)*"
            )
            ...
            try:
                ...
                pass  # If we can't even reply, at least the console log below fires

            print(f"[on_tree_error] discord.Forbidden in /{interaction.command.qualified_name if interaction.command else '?'}: {reason}")
            return  # Handled — don't fall through to the default handler

        # For every other unhandled slash-command error, log it and tell the user
        print(f"[on_tree_error] Unhandled error in slash command: {error}")
        import traceback
        traceback.print_exc()
    ...
    async def on_ready(self):
        ...
        try:
            ...
            print('✅ Stock market update system started (every 30 seconds)')
        except Exception as e:
...
async def main():
    ...
    try:
        ...
        try:
            ...
            # Initialize mega projects
            print('🏗️ Initializing mega projects...')
            await db.initialize_mega_projects()
            print('✅ Mega projects initialized')
        except Exception as e:
