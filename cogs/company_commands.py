            ...
            if forum_id:
                ...
                try:
                    ...
                    if forum and isinstance(forum, discord.ForumChannel):
                        ...
                        permissions = forum.permissions_for(bot_member)
                        
                        if not permissions.create_forum_threads:
                            print("❌ Bot missing CREATE_FORUM_THREADS permission")
                            raise discord.Forbidden(discord.http.HTTPException, "Bot lacks permission to create threads in forum")
                        
                        if not permissions.send_messages:
                            print("❌ Bot missing SEND_MESSAGES permission")
                            raise discord.Forbidden(discord.http.HTTPException, "Bot lacks permission to send messages in forum")
                        ...
                        # Restrict thread to owner-only posting
                        try:
                            print("Setting thread permissions to owner-only...")
                            # Get the owner member
                            owner_member = interaction.guild.get_member(interaction.user.id)
                            
                            if owner_member:
                                # Set permissions: deny @everyone from sending messages, allow owner
                                await thread.edit(
                                    locked=False,  # Keep unlocked so owner can post
                                    archived=False
                                )
                                
                                # Override permissions for the thread
                                overwrites = {
                                ...
                                # Apply permissions to the thread
                                for target, overwrite in overwrites.items():
                                    await thread.set_permissions(target, overwrite=overwrite)
                                
                                print("Thread permissions set successfully")
                        except Exception as perm_error:
                            print(f"Failed to set thread permissions: {perm_error}")
                    ...
                    print(f"Permission error creating thread: {forbidden_error}")
                    # Re-raise with more context
                    raise Exception(f"Bot lacks permissions in forum channel. Please ensure the bot has: Create Forum Threads, Send Messages, and Manage Threads permissions.")
                except Exception as e:
