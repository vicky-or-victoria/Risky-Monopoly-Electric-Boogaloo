...
class EconomyCommands(commands.Cog):
    ...
    @commands.command(name="pay-loan", description="Pay off a loan")
    async def pay_loan(self, ctx, loan_id: int):
        """Pay off a loan (prefix command)"""
        # Get loan
        loan = await db.get_loan_by_id(loan_id)
        
        if not loan:
            return await ctx.send('❌ Loan not found!', delete_after=10)
        
        # Check if user owns the loan
        if loan['borrower_id'] != str(ctx.author.id):
            return await ctx.send('❌ This is not your loan!', delete_after=10)
        
        # Check if already paid
        if loan['is_paid']:
            return await ctx.send('❌ This loan has already been paid!', delete_after=10)
