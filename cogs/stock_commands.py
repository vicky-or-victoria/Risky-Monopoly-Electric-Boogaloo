...
class StockCommands(commands.Cog):
    ...
    async def setup_stock_market(self, interaction: discord.Interaction, channel: discord.TextChannel):
        ...
        print(f"setup-stock-market called by {interaction.user.name}")
        
        print("Checking admin permissions...")
        if not await self.is_admin(interaction):
            print(f"User {interaction.user.name} is not admin")
            await interaction.response.send_message("❌ You don't have permission to use this command.", ephemeral=True)
            return
        
        print("User is admin, deferring response...")
        await interaction.response.defer()
        print("Response deferred")
        
        try:
            print("Starting setup-stock-market...")
            
            # Simplify: Just use initial prices without database calls that might hang
            updates = []
            for symbol, data in STOCK_COMPANIES.items():
                updates.append({
                    'symbol': symbol,
