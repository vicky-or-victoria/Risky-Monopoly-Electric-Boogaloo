...
class StockMarketView(discord.ui.View):
    ...
    async def buy_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        is_frozen = await db.is_stock_market_frozen(str(interaction.guild.id))
        if is_frozen:
            return await interaction.followup.send(
                "🔒 **Stock Market is Frozen**\n"
                "Trading is currently disabled. Please wait for an administrator to unfreeze the market.",
                ephemeral=True
            )
    ...
    async def refresh_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        embed.set_footer(text=f"Requested by {interaction.user.name} • Prices update every 3 minutes")
...
class StockSharesModal(discord.ui.Modal, title="Enter Number of Shares"):
    ...
    async def _process_sell(self, interaction: discord.Interaction, shares: int):
        """Process stock sale with progressive tax"""
        await interaction.response.defer()
        ...
        await db.update_player_balance(str(interaction.user.id), net_proceeds)
        ...
        embed.add_field(name="💰 Gross Sale Value", value=f"${sale_value:,}", inline=True)
        
        # Add tax information
        tax_rate = (base_tax / sale_value * 100) if sale_value > 0 else 0
        embed.add_field(name="📉 Trading Tax", value=f"${final_tax:,} ({tax_rate:.1f}%)", inline=True)
        
        if final_tax < base_tax:
            embed.add_field(
        ...
        embed.add_field(name="💵 Net Proceeds", value=f"${net_proceeds:,}", inline=True)
...
class StockCommands(commands.Cog):
    ...
    async def stock_market(self, interaction: discord.Interaction):
        ...
        if chart_file:
            await interaction.response.send_message(embed=embed, view=view, file=chart_file)
        else:
    ...
    async def setup_stock_market(self, interaction: discord.Interaction, channel: discord.TextChannel):
        ...
        try:
            ...
            chart_file = await generate_stock_chart()
            if chart_file:
                message = await channel.send(embed=embed, view=view, file=chart_file)
            else:
            ...
            embed_success.add_field(
                name="📊 Features",
                value="• Prices update automatically every **3 minutes**\n"
    ...
    @app_commands.command(name="freeze-stock-market", description="🔒 Freeze or unfreeze the stock market (Admin only)")
    @app_commands.describe(
        freeze="True to freeze the market, False to unfreeze it"
    )
    async def freeze_stock_market(self, interaction: discord.Interaction, freeze: bool):
        """Freeze or unfreeze the stock market - admin / owner / trusted role only"""
        if not await is_admin_or_authorized(interaction):
            return await interaction.response.send_message(
                "❌ You don't have permission to use this command.\n"
                "Only the **bot owner**, **server administrators**, or users with an **authorized admin role** can freeze/unfreeze the stock market.",
                ephemeral=True
            )
        ...
        try:
            await db.set_stock_market_frozen(str(interaction.guild.id), freeze)
            
            # Update the stock market display if it exists
            stock_channel_id = await db.get_stock_market_channel(str(interaction.guild.id))
            stock_message_id = await db.get_stock_market_message(str(interaction.guild.id))
            
            if stock_channel_id and stock_message_id:
                try:
                    channel = interaction.guild.get_channel(int(stock_channel_id))
                    if not channel:
                        channel = await interaction.guild.fetch_channel(int(stock_channel_id))
                    ...
                    if channel:
                        ...
                        from stock_market import create_stock_market_embed
                        embed = create_stock_market_embed(updates, freeze)
                        await message.edit(embed=embed)
                except Exception as e:
                    print(f"Error updating stock market display: {e}")
            
            if freeze:
                embed = discord.Embed(
                    title="🔒 Stock Market Frozen",
                    description="The stock market has been **frozen**. All trading is now disabled.",
                    color=discord.Color.red()
                )
                embed.add_field(
                    name="📊 What This Means",
                    value="• Stock prices will **not** update\n"
                          "• Players **cannot** buy or sell stocks\n"
                          "• Portfolio values are locked at current prices\n"
                          "• Use this command again with `freeze: False` to unfreeze",
                    inline=False
                )
            else:
                embed = discord.Embed(
                    title="✅ Stock Market Unfrozen",
                    description="The stock market has been **unfrozen**. Trading is now enabled!",
                    color=discord.Color.green()
                )
                embed.add_field(
                    name="📊 What This Means",
                    value="• Stock prices will resume updating every **3 minutes**\n"
                          "• Players can now buy and sell stocks\n"
                          "• Portfolio values will fluctuate with the market",
                    inline=False
                )
            ...
            print(f"Error freezing/unfreezing stock market: {e}")
            import traceback
            traceback.print_exc()
